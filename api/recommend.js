    ...
    try {
      ...
      if (b.action === 'rank') {
        ...
        output = { update: output.update, question: typeof output.question === 'string' ? output.question.slice(0, 220) : null,
          suggestions: R.words(output.suggestions).slice(0, 5), searchHints: (Array.isArray(output.searchHints) ? output.searchHints : []).filter(h => typeof h.title === 'string' && R.types.includes(h.type)).slice(0, 6).map(h => ({ title: h.title.slice(0, 160), type: h.type })) };
      }
